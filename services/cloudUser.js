...
var userService = {
		...
		try{
			customService.findOne(appId, Collections.User, {
				...
				if (encryptedPassword === user.password) { //authenticate user.
					...
					mongoService.document.save(appId,  [{document:user}]).then(function(document) {
	...
	*/        
		...
		try{
			customService.findOne(appId, Collections.User, {
				...
				if(passwordResetKey === resetKey){
					...
					mongoService.document.save(appId,  [{document:user}]).then(function(user) {
	...
	addToRole: function(appId, userId, roleId,accessList, isMasterKey) {
		...
		try{
			//Get the role
			customService.find(appId, Collections.Role, {_id: roleId}, null, null, 1,0, accessList,isMasterKey).then(function(role) {
				...
				//get the user. 
				customService.find(appId, Collections.User, { _id: userId }, null,null,1,0, accessList,isMasterKey).then(function(user) {
