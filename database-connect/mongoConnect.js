  ...
  var obj = {
    ...
    replSet: function() {
      try {
        ...
        if (global.config.mongo.length === 1) {
          return new Server(
            global.config.mongo[0].host,
            global.config.mongo[0].port
          );
        }
        ...
        for (var i = 0; i < global.config.mongo.length; i++) {
          servers.push(
            new Server(
              global.config.mongo[i].host,
              parseInt(global.config.mongo[i].port)
            )
          );
        }
    ...
    connect: function() {
      ...
      try {
        ...
        mongoClient.connect(global.keys.mongoConnectionString, function(
          err,
          db
        ) {
          if (err) {
