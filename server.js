...
if (global.keys.slackWebHook) {
  ...
  global.winston.add(slack, {
    ...
    icon_url:
      "https://cdn2.iconfinder.com/data/icons/circle-icons-1/64/caution-128.png",
    channel: "#devlogs",
...
global.app.use(
  "*/assets",
  global.express.static(path.join(__dirname, "page-templates/assets"))
);
...
try {
  if (
    fs.statSync("./config/cert.crt").isFile() &&
    fs.statSync("./config/key.key").isFile()
  ) {
    //use https
...
global.app.use(busboyBodyParser());
      ...
      if (ignoreUrl(requestRecvd)) {
        ...
        if (!appKey) {
          return res.status(401).send({
            status: "error",
            message:
              "Key not found. You need to have your Client Key or Master Key in the body or url parameter 'key' when you make this request"
          });
        } else {
          ...
          global.q.all(promises).then(
            function(result) {
              ...
              if (!isInPlan) {
                ...
                return res
                  .status(402)
                  .send("Reached Plan Limit. Upgrade Plan.");
              }
...
global.app.use(function(req, res, next) {
  try {
    ...
    if (req.headers.sessionid) {
      ...
      global.sessionHelper.getSession(req.headers.sessionid, function(
        err,
        session
      ) {
        if (!err) {
...
function attachAPI() {
  try {
    ...
    if (
      ...
      console.log(
        "Failed to attach API's because services not loaded properly."
      );
      return;
    }
    ...
    console.log(
      "CloudBoost Server Started on PORT : " + global.app.get("port")
    );
  } catch (e) {
...
function ignoreUrl(requestUrl) {
  try {
    ...
    var ignoreUrl = [
      //for the routes to check whether the particular service is active/not
      "/api/userService",
      "/api/customService",
      "/api/roleService",
      "/api/status",
...
global.app.get("/", function(req, res) {
  ...
  res.send(
    JSON.stringify({
      ...
      message:
        "This is CloudBoost API. If you're looking for the dashboard. It should be running on port 1440."
    })
  );
});

global.app.get("/getFile/:filename", function(req, res) {
  //for getting any file from resources/
  console.log("Getting any file from resources");
  res.sendFile("resources/" + req.params.filename, { root: __dirname });
});

global.app.set("port", 4730); //SET THE DEFAULT PORT.

//Server kickstart:
http.listen(global.app.get("port"), function() {
  try {
    var filePath = "./config/cloudboost.json";
    _checkFileExists(filePath).then(
      function(data) {
        if (data) {
          try {
      ...
      function(e) {
        console.log("ERROR : Server init error. Trying again");
        console.log(e);
...
function setUpAnalytics() {
  try {
    ...
    if (
      process.env["CLOUDBOOST_ANALYTICS_SERVICE_HOST"] ||
      ...
      if (process.env["IS_STAGING"]) {
        if (process.env["CLOUDBOOST_ANALYTICS_STAGING_SERVICE_HOST"]) {
          global.keys.analyticsUrl =
            "http://" +
            process.env["CLOUDBOOST_ANALYTICS_STAGING_SERVICE_HOST"];
        }
      } else {
        global.keys.analyticsUrl =
          "http://" + process.env["CLOUDBOOST_ANALYTICS_SERVICE_HOST"];
      }
...
function setUpRedis() {
  try {
    ...
    if (
      ...
      console.error(
        "FATAL : Redis Cluster Not found. Use docker-compose from https://github.com/cloudboost/docker or Kubernetes from https://github.com/cloudboost/kubernetes"
      );
    }
    ...
    if (
      ...
      if (process.env["REDIS_SENTINEL_SERVICE_HOST"]) {
        ...
        if (
          ...
          while (
            process.env["REDIS_" + i + "_PORT_6379_TCP_ADDR"] &&
            process.env["REDIS_" + i + "_PORT_6379_TCP_PORT"]
          ) {
            if (i > 1) {
              isCluster = true;
            }
...
function setUpMongoDB() {
  ...
  try {
    ...
    if (
      ...
      console.error(
        "INFO : Not running on Docker. Use docker-compose (recommended) from https://github.com/cloudboost/docker"
      );
    }
    ...
    if (
      process.env["CLOUDBOOST_MONGODB_USERNAME"] &&
      process.env["CLOUDBOOST_MONGODB_PASSWORD"]
    ) {
    ...
    if (
      global.config &&
      global.config.mongo &&
      global.config.mongo.length > 0
    ) {
      if (global.config.mongo[0].username && global.config.mongo[0].password) {
        mongoConnectionString +=
          global.config.mongo[0].username +
          ":" +
          global.config.mongo[0].password +
          "@";
      }
      ...
      for (var i = 0; i < global.config.mongo.length; i++) {
        mongoConnectionString +=
          global.config.mongo[i].host + ":" + global.config.mongo[i].port;
        mongoConnectionString += ",";
      }
    } else {
      if (!global.config) {
      ...
      if (process.env["KUBERNETES_STATEFUL_MONGO_URL"]) {
        ...
        global.config.mongo = process.env["KUBERNETES_STATEFUL_MONGO_URL"]
          .split(",")
          .map(function(x) {
            return {
              host: x.split(":")[0],
        ...
        if (
          ...
          while (
            process.env["MONGO_" + i + "_PORT_27017_TCP_ADDR"] &&
            process.env["MONGO_" + i + "_PORT_27017_TCP_PORT"]
          ) {
    ...
    mongoConnectionString = mongoConnectionString.substring(
      0,
    ...
    console.log(
      "MongoDb connection string:" + global.keys.mongoConnectionString
    );

    if (isReplicaSet) {
      console.log("MongoDB is in ReplicaSet");
      var str =
        "?replicaSet=cloudboost&slaveOk=true&maxPoolSize=200&ssl=false&connectTimeoutMS=30000&socketTimeoutMS=30000&w=1&wtimeoutMS=30000";
...
function attachCronJobs() {
  try {
    ...
    global.app.use(function(req, res) {
      res.json({
        status: 404,
        message: "The endpoint was not found. Please check."
      });
    });
  } catch (err) {
    global.winston.log("error", {
